...
KineticSession* create_session(KineticClient * client)
{
    // Configure and establish a session with the specified device
    KineticSession* session;
    const char HmacKeyString[] = "asdfasdf";
...
void test_kinetic_client_throughput_for_small_sized_objects(void)
{
    ...
    // TODO: code to mimic ceph
    KineticSession* testSession = create_session(client);
    KineticClient_DestroySession(testSession);
    KineticSession* dormantSession = create_session(client);
    (void)dormantSession; // to be used for out-of-band operations
    ...
    // Leak the client and rely on process shutdown to cleanup resources, since that what Ceph does
