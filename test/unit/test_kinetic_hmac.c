...
void test_KineticHMAC_Init_should_initialize_the_HMAC_struct_with_the_specified_algorithm(void)
{
    ...
    const Com_Seagate_Kinetic_Proto_Command_Security_ACL_HMACAlgorithm expectedAlgorithm =
        COM_SEAGATE_KINETIC_PROTO_COMMAND_SECURITY_ACL_HMACALGORITHM_HmacSHA1;
...
void test_KineticHMAC_Init_should_set_HMAC_algorithm_to_INVALID_if_the_specified_algorithm_is_invalid(void)
{
    ...
    actual.algorithm = COM_SEAGATE_KINETIC_PROTO_COMMAND_SECURITY_ACL_HMACALGORITHM_HmacSHA1;

    KineticHMAC_Init(&actual, (Com_Seagate_Kinetic_Proto_Command_Security_ACL_HMACAlgorithm)100);

    TEST_ASSERT_EQUAL(COM_SEAGATE_KINETIC_PROTO_COMMAND_SECURITY_ACL_HMACALGORITHM_INVALID_HMAC_ALGORITHM, actual.algorithm);
}

void test_KineticHMAC_Populate_should_compute_and_populate_the_SHA1_HMAC_for_the_supplied_message_and_key(void)
{
    KineticHMAC actual;
    Com_Seagate_Kinetic_Proto_Message msg = COM_SEAGATE_KINETIC_PROTO_MESSAGE__INIT;
    Com_Seagate_Kinetic_Proto_Message_HMACauth hmacAuth = COM_SEAGATE_KINETIC_PROTO_MESSAGE_HMACAUTH__INIT;
    ...
    KineticHMAC_Init(&actual, COM_SEAGATE_KINETIC_PROTO_COMMAND_SECURITY_ACL_HMACALGORITHM_HmacSHA1);
...
{
    KineticHMAC actual;
    Com_Seagate_Kinetic_Proto_Command_Status status = COM_SEAGATE_KINETIC_PROTO_COMMAND_STATUS__INIT;
    Com_Seagate_Kinetic_Proto_Command command = COM_SEAGATE_KINETIC_PROTO_COMMAND__INIT;
    status.code = COM_SEAGATE_KINETIC_PROTO_COMMAND_STATUS_STATUS_CODE_NO_SPACE;
    status.has_code = true;
    command.status = &status;
    Com_Seagate_Kinetic_Proto_Message proto = COM_SEAGATE_KINETIC_PROTO_MESSAGE__INIT;
    Com_Seagate_Kinetic_Proto_Message_HMACauth hmacAuth = COM_SEAGATE_KINETIC_PROTO_MESSAGE_HMACAUTH__INIT;
    uint8_t data[KINETIC_HMAC_MAX_LEN];
    ProtobufCBinaryData hmac = {.len = KINETIC_HMAC_MAX_LEN, .data = data};
    const ByteArray key = ByteArray_CreateWithCString("1234567890ABCDEFGHIJK");
    ...
    size_t packedLen = com_seagate_kinetic_proto_command__pack(&command, packedCmd);
    ...
    proto.authType = COM_SEAGATE_KINETIC_PROTO_MESSAGE_AUTH_TYPE_HMACAUTH;
